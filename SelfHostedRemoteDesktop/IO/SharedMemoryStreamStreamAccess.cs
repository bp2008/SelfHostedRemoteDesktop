	...
	public class SharedMemoryStream : Stream, IDisposable
	{
		...
		private SharedMemoryStream(bool isOwnerProcess, string uniqueId)
		{
			...
			if (isOwnerProcess)
			{
				...
				viewStream = _mmf.CreateViewStream(0, memoryMappedFileSize);
		...
		{
			Util.TryCatch(() => { viewStream?.Dispose(); });
		...
		public override int Read(byte[] buffer, int offset, int count)
		{
			...
			do
			{
				...
				if (amountToRead > 0)
				{
					...
					try
					{
						viewStream.Position = actualReadPosition;
						int amountRead = viewStream.Read(buffer, offset + totalAmountRead, amountToRead);
						myBytesRead += (ulong)amountRead;
						totalAmountRead += amountRead;
						if (amountRead != amountToRead)
							throw new Exception("Read " + amountRead + " from viewStream, but meant to read " + amountToRead + ". Possible logic error.");
					}
					finally
					{
						SetBytesReadByMe();
			...
			if (totalAmountRead != count)
				throw new Exception("Total amount read from viewStream (" + totalAmountRead + ") did not match count (" + count + "). Possible logic error.");
			return count;
		}
		...
		public override void Write(byte[] buffer, int offset, int count)
		{
			...
			do
			{
				...
				if (amountToWrite > 0)
				{
					...
					try
					{
						viewStream.Position = actualWritePosition;
						viewStream.Write(buffer, offset + totalAmountWritten, amountToWrite);
			...
			if (totalAmountWritten != count)
				throw new Exception("Total amount written to viewStream (" + totalAmountWritten + ") did not match count (" + count + "). Possible logic error.");
		}
		...
		private ulong GetBytesWrittenByRemote()
		{
			...
			try
			{
				viewStream.Position = isOwnerProcess ? 24 : 8;
				byte[] buf = new byte[sizeof(ulong)];
				viewStream.Read(buf, 0, buf.Length);
		...
		private void SetBytesWrittenByMe()
		{
			...
			try
			{
				viewStream.Position = isOwnerProcess ? 8 : 24;
				byte[] buf = BitConverter.GetBytes(myBytesWritten);
				viewStream.Write(buf, 0, buf.Length);
			}
			finally
			{
				_ewh_write.Set();
		...
		private ulong GetBytesReadByRemote()
		{
			...
			try
			{
				viewStream.Position = isOwnerProcess ? 16 : 0;
				byte[] buf = new byte[sizeof(ulong)];
				viewStream.Read(buf, 0, buf.Length);
				return BitConverter.ToUInt64(buf, 0);
			}
		...
		private void SetBytesReadByMe()
		{
			...
			try
			{
				viewStream.Position = isOwnerProcess ? 0 : 16;
				byte[] buf = BitConverter.GetBytes(myBytesRead);
				viewStream.Write(buf, 0, buf.Length);
			}
			finally
			{
